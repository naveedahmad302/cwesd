...
interface SharedCalendarScreenProps {
  userRole: 'student' | 'teacher';
  courseId?: string;
}

const SharedCalendarScreen: React.FC<SharedCalendarScreenProps> = ({ userRole, courseId = '6' }) => {
  ...
  const getDaysWithEvents = () => {
    return events.reduce((days: Set<number>, event: Event) => {
      const eventDate = new Date(event.start);
      if (
...
const styles = StyleSheet.create({
  ...
  currentDaySection: {
    ...
    shadowRadius: 4,
    elevation: 3,
    minHeight: 120, // Minimum height for section
  },
  currentDayHeader: {
    flexDirection: 'row',
    alignItems: 'center',

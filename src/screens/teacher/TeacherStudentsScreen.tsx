...
import { View, StyleSheet, ScrollView, TouchableOpacity, TextInput, ActivityIndicator, Image, RefreshControl } from 'react-native';
...
const TeacherStudentsScreen = () => {
  ...
  const students = useMemo(() => {
    const raw = (studentsResponse as any)?.data ?? studentsResponse?.data;
    ...
    return data.map((student: any) => ({
      ...
      batch: student.enrolledCourses?.length > 0
  ...
  const errorMessage = isError && queryError
    ? (queryError as { data?: { message?: string }; message?: string })?.data?.message ?? (queryError as { message?: string })?.message ?? 'Failed to load students'
    : null;
